...
const displayColors = () => {
  Object.keys(colours).forEach((key) => {
    ...
    option.text = key;
    colorNames.add(option);
  });

  text.textContent = colours["aliceblue"];
  circle.style.backgroundColor = colours["aliceblue"];
};

const setColor = () => {
  text.textContent = colours[colorNames.value];
  circle.style.backgroundColor = colorNames.value;
};
...
colorNames.addEventListener("change", setColor);
